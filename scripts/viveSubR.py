#************** This is the vive subscriber for the right hand position 
...
msg1 = JointState()
psm1_joint_angles = rospy.Publisher('/dvrk_psm1/joint_states_robot', JointState, queue_size=1) #right psm
vis_pub = rospy.Publisher("/key_holeRM", Marker, queue_size=1)
...
# Kinematics Solvers
psm1_kin = KDLKinematics(psm1_robot,psm1_robot.links[0].name,psm1_robot.links[11].name)
...
def cb(d):	
	...
	global qList
	#print '************************* \n   Function Called\n*************************'
	initialFrame=psm1_kin.forward(qList)  # Do the forward Kinematics for the initial angles
...
#      The desired frame is the initial frame plus the delta
	...
	q_out = psm1_kin.inverse(desiredFrame,qList,q_min,q_max)
...
def listener():
	rospy.init_node('rightJointPub', anonymous=True)
#	rospy.Subscriber("deltas", Twist, cb)	
	rospy.Subscriber("/vive/right_hand", Twist, cb,queue_size=1) 	# queue size must be 1 or it lags and
